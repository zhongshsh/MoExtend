...
from .visual import CLIPVisionTower
...
def make_context(
    ...
    if chat_format == "chatml":
        ...
        raw_text = f"{im_start}{query}{im_end}"
...
def load_balancing_loss_func(
    ...
    overall_loss = 0
    median = int(len(gate_logits) / 2)
    quarter = int(len(gate_logits) / 4)
    for i, layer_gate in enumerate(gate_logits):
        num_experts_ = num_experts - 1 if i not in add_layer else num_experts

        routing_weights = torch.nn.functional.softmax(layer_gate, dim=-1).to(
            compute_device
        )
        _, selected_experts = torch.topk(routing_weights, top_k, dim=-1)

        expert_mask = torch.nn.functional.one_hot(selected_experts, num_experts_)
        ...
        if attention_mask is None:
            ...
            num_hidden_layers = selected_experts.shape[0] // (
            ...
            expert_attention_mask = (
                ...
                .expand(
                    (num_hidden_layers, batch_size, sequence_length, 2, num_experts_)
                )
                .reshape(-1, 2, num_experts_)
                .to(compute_device)
            )
            ...
            router_per_expert_attention_mask = (
                ...
                .expand((num_hidden_layers, batch_size, sequence_length, num_experts_))
                .reshape(-1, num_experts_)
                .to(compute_device)
            )
            # Compute the average probability of routing to these experts
...
class WeightScale(nn.Module):
    def __init__(self):
        ...
        self.weight_scale = nn.Sequential(
            nn.Linear(9, 16, bias=False),
            nn.GELU(),
            nn.Linear(16, 9, bias=False),
        )
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, idx=0, use_gate_vision=True):
        super().__init__()
        self.hidden_dim = config.hidden_size
        self.ffn_dim = config.intermediate_size
        self.num_experts = config.num_local_experts
        self.top_k = config.num_experts_per_tok
        self.use_gate_vision = use_gate_vision

        # gating
        self.gate = nn.Linear(self.hidden_dim, self.num_experts - 1, bias=False)
        if self.use_gate_vision:
            self.gate_vision = nn.Linear(self.hidden_dim, 1, bias=False)
...
class MixtralDecoderLayer(nn.Module):
    def __init__(self, config: MixtralConfig, layer_idx: int, use_gate_vision=True):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.block_sparse_moe = MixtralSparseMoeBlock(
            config, idx=layer_idx, use_gate_vision=use_gate_vision
        )
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def __init__(self, config: MixtralConfig):
        ...
        for layer_idx in range(config.num_hidden_layers):
            if layer_idx in config.add_layer:
                print(f"Add extra expert in layer {layer_idx}")
                module.append(
                    MixtralDecoderLayer(config, layer_idx, use_gate_vision=True)
                )
            else:
                module.append(
                    MixtralDecoderLayer(config, layer_idx, use_gate_vision=False)
                )
    ...
    def forward(
        ...
        if fake_images is not None:
            hidden_states = hidden_states + images.to(hidden_states.device).mean() * 0
...
class MixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.model.visual.load_model()

    def _init_vision_expert(self, vision_encode=False, **kwargs):
        for name, param in self.named_parameters():
            param.requires_grad = False

        for mixtral_decoder in self.model.layers:
            if mixtral_decoder.block_sparse_moe.use_gate_vision:
                mixtral_decoder.block_sparse_moe.weight_scale.requires_grad_(True)
                mixtral_decoder.block_sparse_moe.gate_vision.requires_grad_(True)
                mixtral_decoder.block_sparse_moe.experts[-1].requires_grad_(True)

        self.model.visual.mm_projector.requires_grad_(vision_encode)

    def _tran_vision_expert(self, layer2expert, **kwargs):

        for i, mixtral_decoder in enumerate(self.model.layers):
            if i in self.config.add_layer:
                std = self.config.initializer_range
                mixtral_decoder.block_sparse_moe.weight_scale.weight_scale[
                    0
                ...
                print(f"Layer {i} new expert copy from expert {layer2expert[i]}")
                state_dict = mixtral_decoder.block_sparse_moe.experts[
                    layer2expert[i]
                ].state_dict()
                mixtral_decoder.block_sparse_moe.experts[-1].load_state_dict(state_dict)

                state_dict = mixtral_decoder.block_sparse_moe.gate.weight
                mixtral_decoder.block_sparse_moe.gate_vision.weight.data = (
                    state_dict.data[layer2expert[i], :].unsqueeze(0).detach().clone()
                )
